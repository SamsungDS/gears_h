...
class BondCenteredTensorMomentDescriptor(nn.Module):
  ...
  @nn.compact
  def __call__(self, atomic_descriptors, neighbour_displacements, neighbours_i, neighbours_j):

    num_radial_features = atomic_descriptors.shape[-1]

    atom1_desc, atom2_desc = atomic_descriptors[neighbours_i], atomic_descriptors[neighbours_j]
